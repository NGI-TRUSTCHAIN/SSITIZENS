...
def password_reset_token_created(sender, instance, reset_password_token, **kwargs):
    ...
    send_email.delay(
        _("Password reset for Ssitizens"),
...
def post_save_user_profile(sender, instance: User, **kwargs):
    if instance.is_superuser and instance.email is not None and kwargs.get("created"):
        subject = _("Ssitizens Account Activation")
        token_service = PasswordResetTokenGenerator()
        welcome_message = _(
            str(
                "You have been invited to the Ssitizens Platform. In order to continue you need to activate your account and update your password through the form available at the following link: "
            )
        )
